...
if zone == "Maj'Eyal" then
	...
	wda.cur_zigur_patrols = wda.cur_zigur_patrols or 0
    wda.cur_adv_parties = wda.cur_adv_parties or 0
	...
	-- Spawn some Ziguranth patrols
	if wda.cur_zigur_patrols < game.state:canZiguranthPatrol() and rng.percent(5) then
		local e = game.zone:makeEntity(game.level, "maj_eyal_encounters_npcs", {name="ziguranth patrol"}, nil, true)
		if e then
			local spot = game.level:pickSpot{type="hostile", subtype="maj-eyal"}
			if spot and not game.level.map(spot.x, spot.y, Map.ACTOR) and not game.level.map.seens(spot.x, spot.y) then
				print("Spawned Ziguranth patrol", spot.x, spot.y, e.name)
				game.zone:addEntity(game.level, e, "actor", spot.x, spot.y)
				wda.cur_zigur_patrols = wda.cur_zigur_patrols + 1
				e.world_zone = zone
				e.on_die = function(self) game.level.data.wda.zones[self.world_zone].cur_zigur_patrols = game.level.data.wda.zones[self.world_zone].cur_zigur_patrols - 1 end
			end
		end
	end
    
	-- Spawn some adventurer parties
	if wda.cur_zigur_patrols + wda.cur_adv_parties < 5 and rng.percent(5) then
		local e = game.zone:makeEntity(game.level, "maj_eyal_encounters_npcs", {name="adventurers party"}, nil, true)
		if e then
			local spot = game.level:pickSpot{type="hostile", subtype="maj-eyal"}
			if spot and not game.level.map(spot.x, spot.y, Map.ACTOR) and not game.level.map.seens(spot.x, spot.y) then
				print("Spawned adventurers party", spot.x, spot.y, e.name)
				game.zone:addEntity(game.level, e, "actor", spot.x, spot.y)
				wda.cur_adv_parties = wda.cur_adv_parties + 1
				e.world_zone = zone
				e.on_die = function(self) game.level.data.wda.zones[self.world_zone].cur_adv_parties = game.level.data.wda.zones[self.world_zone].cur_adv_parties - 1 end
			end
		end
	end
